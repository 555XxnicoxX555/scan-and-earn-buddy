...
- **Migración remota:** `supabase/migrations/20260824000100_audited_reward_redemptions.sql` sigue sin aplicarse al proyecto remoto; el manifiesto de Supabase está preparado, pero falta aprobación explícita de la identidad del proyecto y del preflight read-only.
...
| La migración auditada sigue sin aplicar al proyecto remoto; el manifiesto de Supabase está listo, pero la identidad del proyecto y el preflight read-only requieren aprobación explícita antes de ejecutarse. | `supabase/verification/20260824000100_audited_reward_redemptions_{preflight,postflight}.sql` y manifiesto de ejecución remoto. | 2026-08-24 |
...
| Canjes auditables | `redemption_implementer` | Implementación local completada; auditorías estáticas, build y smoke aislado completo pasan; revisión precommit sin P0/P1 locales; aplicación remota pendiente | `app.js`, `index.html`, `styles.css`, `ADMIN_PANEL.md`, `supabase/migrations/20260824000100_audited_reward_redemptions.sql` | Aprobación de identidad/preflight y después apply/postflight Supabase |
| Auditoría de seguridad y datos | Especialistas de seguridad/datos + root | Revisión precommit sin P0/P1 locales; el P1 remoto queda pendiente por la migración no aplicada | Reportes de revisión y árbol local corregido | Validación de Supabase y pruebas remotas autorizadas |
...
| Supabase/RLS/migraciones | Root + especialista de datos | Nueva migración de canjes auditables presente localmente; dashboard figura Sumi; manifiesto listo; identidad/preflight y apply remoto pendientes; caché local conserva nombre histórico | `supabase/migrations/**`, `supabase/config.toml`, `supabase/verification/**` | Aprobación de identidad/preflight, luego mutación y postflight |
...
| Aplicar migraciones en Supabase | Escritura de esquema, funciones, RLS y posiblemente Storage; no es una prueba simple | Backup/plan de recuperación, migración idempotente, registrar resultado y proyecto exacto | Requiere aprobación explícita de mutación | Pendiente; `20260824000100_audited_reward_redemptions.sql` está sólo en el árbol local |
...
| Deploy Vercel / conectar DNS Hostinger | Mutaciones externas y publicación | Seguir `docs/DEPLOYMENT_RUNBOOK.md`: commit aprobado, importación root `.`, Vite/build `dist`, variables por entorno, preview/logs, dominio exacto, TTL y rollback; no borrar Lovable | Usuario autorizó el objetivo general, pero se exige aprobación de preview y confirmación de acción destructiva/DNS en el momento | Pendiente; no ejecutado |
...
- **Despliegue:** [runbook recuperable](docs/DEPLOYMENT_RUNBOOK.md), basado en `vercel.json`, `.gitignore`, `.env.example` y `scripts/build.mjs`; no ejecutado.
...
- **Supabase:** el dashboard visible figura como Sumi; la caché local aún es histórica; `20260824000100_audited_reward_redemptions.sql` está presente localmente, pero no hay evidencia en este ledger de que las migraciones pendientes se hayan aplicado al proyecto remoto.
- **Pruebas:** `audit:ui` (153) y `audit:redemptions` (16) pasan; build principal/onboarding pasa; smoke aislado completo pasa con Supabase desactivado, verifica `remoteDisabled`/`hasSupabase` y cierra el listener. La identidad/preflight remoto de Supabase no se ejecutó por falta de aprobación.
- **Riesgos residuales:** además de las validaciones y migración de canjes pendientes, el proyecto Vercel final todavía no está confirmado/importado, no hay preview ni deployment ID de Sumi, las variables por entorno no están verificadas, DNS no cambió y Lovable no se retiró; persisten la linked-project cache histórica, cambios mezclados de skills/`graphify-out/` y la arquitectura freelance pendiente de implementación.
- **Recomendación de entrega:** la puerta local ya está cerrada con evidencia; seguir ahora el manifiesto remoto (identidad/preflight aprobado) y luego el runbook: commit aprobado, importación Vercel root `.`, build/preview/logs, variables sin `service-role`, dominio con registros exactos y TTL, validación HTTPS/QR/Auth, registro de deployment/rollback y sólo después la decisión destructiva sobre Lovable. El root debe actualizar el objetivo oficial; el `documenter` no lo crea ni lo modifica.
